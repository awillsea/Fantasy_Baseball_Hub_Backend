...
namespace Fantasy_Baseball_Hub_Backend.Models.Logic
{
    ...
    {
        public static void ScrapeFanGraphsAdvancePitchingStats()
        {
            ...
            {
                string url = $"https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=0&type=8&season=2023&month=0&season1=2023&ind=0&team=0&rost=0&age=0&filter=&players=0&startdate=2023-01-01&enddate=2023-12-31&page={i}_30";
            ...
            {   // created a Tuple so my Functions FindPlayerIDANdPostion() can take in one argument but return two results, the ID and Postion
                ...
                newPlayer.Saves = Int32.Parse(player[5]);
                newPlayer.GamesPlayed = Int32.Parse(player[6]);
                newPlayer.GamesStarted = Int32.Parse(player[7]);
                newPlayer.InningsPitched = Decimal.Parse(player[8]);
                newPlayer.StrikeoutsPerNineInnings= Decimal.Parse(player[18]);
                newPlayer.WalksPerNineInnings= Decimal.Parse(player[9]);
                newPlayer.HomeRunsPerNineInnings = Decimal.Parse(player[10]);
                newPlayer.BattingAverageOnBallsInPlay= Decimal.Parse(player[11]);
                newPlayer.LeftOnBasePercentage = Decimal.Parse(player[12].TrimEnd(new char[] { '%', ' ' }));
                newPlayer.GroundBallPercentage= Decimal.Parse(player[13].TrimEnd(new char[] { '%', ' ' }));
                newPlayer.HomeRunToFlyBallRation= Decimal.Parse(player[14].TrimEnd(new char[] { '%', ' ' }));
                //newPlayer.AverageFastBallVelocity = Decimal.Parse(player[19]);
                newPlayer.EarnedRunAverage= Decimal.Parse(player[20]);
                //newPlayer.ExpectedEarnRunAverage = Decimal.Parse(player[15]);
                newPlayer.FieldingIndependentPitching = Decimal.Parse(player[16]);
                newPlayer.ExpectedFeildingIndependentPitching= Decimal.Parse(player[17]);
                newPlayer.WinsAboveReplacement= Decimal.Parse(player[21]);


                if (player[19] == "&nbsp;")
                {
                    newPlayer.AverageFastBallVelocity = 0m;
                }
                else
                {
                    newPlayer.AverageFastBallVelocity = Decimal.Parse(player[19]);
