...
namespace Fantasy_Baseball_Hub_Backend.Models.Logic
{
    ...
    {
       public static void ScrapeFanGraphsStandardHittingStats( )
       {
                ...
                {   // created a Tuple so my Functions FindPlayerIDANdPostion() can take in one argument but return two results, the ID and Postion
                    ...
                    newPlayer.AtBats = Int32.Parse(player[4]);
                    newPlayer.PlateApperances = Int32.Parse(player[5]);
                    ...
                    newPlayer.HomeRuns = Int32.Parse(player[10]);
                    newPlayer.Runs = Int32.Parse(player[11]);
                    newPlayer.RBI = Int32.Parse(player[12]);
                    newPlayer.Walks = Int32.Parse(player[13]);
                    newPlayer.IntentionalWalks = Int32.Parse(player[14]);
                    newPlayer.StrikeOuts = Int32.Parse(player[15]);
                    newPlayer.HitByPitch = Int32.Parse(player[16]);
                    newPlayer.SacraficeFly = Int32.Parse(player[17]);
                    newPlayer.SacraficeHit = Int32.Parse(player[18]);
                    newPlayer.GroundIntoDoublePlays = Int32.Parse(player[19]);
                    newPlayer.StolenBase = Int32.Parse(player[20]);
                    newPlayer.CaughtStealing = Int32.Parse(player[21]);
                    newPlayer.BattingAverage = Decimal.Parse(player[22]);
